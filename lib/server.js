...
module.exports = function(args) {
  ...
  return checkPort(ip, port).then(() => this.extend.filter.exec('server_middleware', app, {context: this})).then(() => {
    ...
    this.log.info(`Hexo is running at ${chalk.underline(addrString)} . Press Ctrl+C to stop.`);
    this.emit('server');
...
function checkPort(ip, port) {
  if (port > 65535 || port < 1) {
    return Promise.reject(new Error(`Port number ${port} is invalid. Try a number between 1 and 65535.`));
  }
